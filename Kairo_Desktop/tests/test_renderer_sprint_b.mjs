...
console.log('\n--- T14: IPC channels ---')
{
  ...
  assert(src.includes("FS_READ_FILE: 'fs:read-file'"), 'T14f: FS_READ_FILE channel exists')
  assert(src.includes("FS_WRITE_FILE: 'fs:write-file'"), 'T14g: FS_WRITE_FILE channel exists')
  assert(src.includes("FS_LIST_DIR: 'fs:list-dir'"), 'T14h: FS_LIST_DIR channel exists')
  ...
  assert(channels.length === 48, `T14c: 48 channels total (got ${channels.length})`)
}
