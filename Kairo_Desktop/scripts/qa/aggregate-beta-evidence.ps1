...
foreach ($ef in $evidenceFiles) {
    ...
    $machine = @{
        ...
        NodeVersion = '(unknown)'
        AppVersion = '(unknown)'
        ElectronVersion = '(not detected)'
        InstallerHash = ''
        InstallerSize = '(not found)'
        NativeModules = @()
        HasLogs = $false
    }
...
if ($evidenceCount -eq 0) {
    ...
    foreach ($m in $machines) {
        ...
        $nativeMods = if ($m.NativeModules.Count -gt 0) { ($m.NativeModules -join ', ') } else { 'none detected' }
        $electronVal = if ($m.ElectronVersion -and $m.ElectronVersion -ne '(not detected)') { $m.ElectronVersion } else { '(not detected)' }
        $md += "| $($m.MachineName) | $($m.OS) | $($m.NodeVersion) | $($m.AppVersion) | $electronVal | $hashShort | $nativeMods |"
    }
    ...
    foreach ($m in $machines) {
        ...
        $md += "- **Electron**: $(if ($m.ElectronVersion -and $m.ElectronVersion -ne '(not detected)') { $m.ElectronVersion } else { '(not detected)' })"
        $md += "- **Installer Size**: $(if ($m.InstallerSize -and $m.InstallerSize -ne '(not found)') { $m.InstallerSize } else { '(not found)' })"
        $md += "- **Installer SHA256**: $(if ($m.InstallerHash) { '``' + $m.InstallerHash + '``' } else { '(not available)' })"
...
$warnings = @()
$noHash = @($machines | Where-Object { -not $_.InstallerHash })
if ($noHash.Count -gt 0) { $warnings += "- $($noHash.Count) report(s) missing installer SHA256 hash." }
$noElectron = @($machines | Where-Object { $_.ElectronVersion -eq '(not detected)' })
if ($noElectron.Count -gt 0) { $warnings += "- $($noElectron.Count) report(s) missing Electron version (node_modules not installed?)." }
$noNative = @($machines | Where-Object { $_.NativeModules.Count -eq 0 })
if ($noNative.Count -gt 0) { $warnings += "- $($noNative.Count) report(s) with no native modules detected (app.asar.unpacked missing?)." }
...
$md += "_Evidence collected via ``collect-beta-evidence.ps1`` on each tester machine._"
